...
#consumption, exports, imports, production, purchases
...
# Sum UK seafood supplies and intake per capita for each year
df_UK_supplies_avg <- aggregate(Supplies_capWeek ~ Year, data = df_UK_seafoodDB_capita, sum)
df_UK_intake_avg <- aggregate(Consumption_capWeek ~ Year, data = df_UK_seafoodDB_capita, sum)

# Merge data frames
df_UK_supplies_intake_avg <- merge(df_UK_supplies_avg,df_UK_intake_avg, by = "Year" )

# Plot average seafood supply (g/capita/week) between 2009 and 2018 
ggplot(df_UK_supplies_intake_avg, aes(Year, Supplies_capWeek)) +
  geom_point(size = 3) +
  scale_y_continuous(name = "Seafood supplies (g/capita/week)", limits = c(0,300)) +
  geom_hline(yintercept = 280, linetype = "dashed", color = "red", linewidth = 1.5) +
  geom_point(aes(y= Consumption_capWeek, x= Year), col = "blue", size = 3) +
  theme(legend.position = "right") +
  scale_colour_discrete(breaks=c("supplies","intake"))
...
```{r Dietary recommendations on same plot}
# Remove NA
#df_UK_supplies_SACN <- subset()
...
# a) Oily and lean fish only 

# Subset lean and oily fish only 
df_UK_SACN_LO_avg <- subset(df_UK_supplies_SACN_avg, SACN == "Lean" | SACN == "Oily")
df_UK_SACN_LO_avg$Type <- "Seafood type"

# Specify order the bars should be in (from top to bottom) in the stacked bar chart
# Convert 'SACN' to factor and specify level order
df_UK_SACN_LO_avg$SACN <- factor(df_UK_SACN_LO_avg$SACN, levels = c("Lean", "Oily"))
levels(as.factor(df_UK_SACN_LO_avg$SACN))


# Scotland 
plot_ScotRecStack <- ggplot(df_UK_SACN_LO_avg, 
                            aes(y = Supplies_capWeek, x= Type, 
                                fill = factor(SACN, levels = c("Lean", "Oily"))))+
  geom_bar(position = "stack", stat = "identity", color = "black") +
  scale_y_continuous(name = "Seafood supplies (g/capita/week)", limits = c(0,300)) +
  geom_hline(yintercept = 140, linetype = "dashed", color = "red", linewidth = 1.0) 

# UK 
plot_UKRecStack <- ggplot(df_UK_SACN_LO_avg, 
                            aes(y = Supplies_capWeek, x= Type, 
                                fill = factor(SACN, levels = c("Lean", "Oily"))))+
  geom_bar(position = "stack", stat = "identity", color = "black") +
  scale_y_continuous(name = "Seafood supplies (g/capita/week)", limits = c(0,300)) +
  geom_hline(yintercept = 280, linetype = "dashed", color = "red", linewidth = 1.0) +
  labs(x = NULL) + 
  scale_colour_manual("",  breaks = c("Lean", "Oily"), values = c("#D55E00", "#CC79A7"))

# Colours to use: "#D55E00", "#CC79A7", "#999999"
# Combine plots
plot_grid(plot_ScotRecStack, plot_UKRecStack)
```
...
df_UK_SOI_capita_2018 <- subset(df_UK_seafoodDB_capita_2018, Species == "Cod" & Species == "Haddock" & 
...
### Figure. Barchart of seafood database commodities 
