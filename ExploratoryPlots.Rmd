...
```{r Load the database}
# Load database 
df_UK_seafoodnurient <- vroom(file=paste(filepath,"NutrientComp_PHENor_PreliminaryCleaned.csv", sep="/"), na = c("NA"))
...
df_UK_seafoodDB_food_spec <- df_UK_seafoodDB[c("Species", "SpeciesType", "SACN", "Year", "Commodity", 
                                                    "Volume" )]

# Aggregate value by commodity, year and species (production data composed of aquaculture and wild capture)
df_UK_seafoodDB_food_simp <- aggregate(Volume ~ Commodity + Year + Species + SpeciesType + SACN, data = df_UK_seafoodDB_food_spec, sum)
...
## Calculate UK seafood supplies (g/capita/week) and (g/capita/day)
...
df_UK_seafoodDB_capita$Consumption_capDay <- (df_UK_seafoodDB_capita$Consumption/ df_UK_seafoodDB_capita$UKPOP)/365
...
df_UK_seafoodnurient_simp <- df_UK_seafoodnurient[,c("Species", "Nutrient", "Volume")]

# Unstack nutrients data 
# Make data frame wide and short
df_nutrient_short <- df_UK_seafoodnurient_simp %>%
  pivot_wider(names_from = Nutrient, values_from = Volume)

# Merge nutrient data with seafood database
df_seafood_CompDB <- merge(df_UK_seafoodDB_capita, df_nutrient_short, "Species")
...
# Plot bars side by side
ggplot(df_UK_seafoodDB_food, aes(x = SACN, y = Value, fill = Commodity)) +
  geom_col(position = "dodge")

# Remove "No data" from category 
df_UK_seafoodDB_SACN<- subset(df_UK_seafoodDB_food, SACN != "NoData")

ggplot(df_UK_seafoodDB_SACN, aes(x = SACN, y = Value, fill = Commodity)) +
  geom_col(position = "dodge")
...
# Create a list
list_seafood_df <- list(df_UK_seafood_prod, df_UK_seafood_imp, df_UK_seafood_exp, df_UK_seafood_purch, df_UK_seafood_consump)

g <- Reduce(function(x, y) merge(x, y, all=TRUE), list_seafood_df)
...
# Box plot of UK seafood supplies. Base R
...
```{r Box plot of certain seafood species (g/capita/week)}
# Subset oily and lean fish
# Merge UK seafood supplies with population data

# Divide supplies by population size (per capita)

# Divide by 52 (per capita, per week)

# Aggregate supplies by species
df_UK_supplies_SACN <- aggregate(supplies ~ SACN, data = df_UK_supplies_short, sum)


# Plot by seafood supplies by SACN category.
# Include estimated consumption of the categories and the dietary recommendation (140g)
suppliesPerSACN <- ggplot(df_UK_supplies_SACN, aes(SACN, supplies)) +
  geom_col() +
  scale_x_discrete(guide = guide_axis(angle = 90), name = "") + 
  scale_y_continuous(name = "Seafood supplies (g/capita/week)") +
  geom_hline(yintercept = 140, linetype = "dashed", color = "red", size = 1.0) + 
  theme_classic() # axis lines but no grid line

# Alter size of the axis text
suppliesPerSACN + theme(
...
```{r Barplot of UK seafood supplies in 2018}
# Subset for most recent year (2018)
df_UK_supplies_2018_all <- subset(df_UK_supplies_short, Year == 2018)
unique(df_UK_supplies_2018$Year)

# Aggregate supplies by species
df_UK_supplies_2018 <- aggregate(supplies~ Species, data = df_UK_supplies_2018_all, sum)

# Plot points of seafood supplies
ggplot(df_UK_supplies_2018, aes(Species, supplies)) +
  geom_point() +
  scale_x_discrete(guide = guide_axis(angle = 90), name = "") + 
  scale_y_continuous(name = "Seafood supplies (g)") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 1.0)
...
# Plot bar chart of seafood supplies
ggplot(df_UK_supplies_2018, aes(Species, supplies)) +
  geom_col() +
  scale_x_discrete(guide = guide_axis(angle = 90), name = "") + 
  scale_y_continuous(name = "Seafood supplies (g)") +
...
purchases_18 <- subset(df_UK_seafood_purch, Year == 2018)
df_UK_seafood_consump_18 <- subset(df_UK_seafood_consump, Year == 2018)
str(df_UK_seafood_consump_18)

df_UK_supConsump_2018 <- merge(df_UK_supplies_2018, df_UK_seafood_consump_18, by = c("Species"), all= TRUE)

df_UK_seafood_2018 <- subset(df_UK_seafoodDB_food, Year == 2018)



colnames(df_UK_supConsump_2018)

# Plot bar chart of seafood supplies
ggplot(df_UK_supConsump_2018, aes(Species, supplies)) +
  geom_col() +
  scale_x_discrete(guide = guide_axis(angle = 90), name = "") + 
  scale_y_continuous(name = "Seafood supplies (g)") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 1.0) + 
...
# Plot bar chart of seafood supplies with colour shading 
ggplot(df_UK_supplies_2018, aes(x= Species, y= supplies, fill= supplies)) + 
