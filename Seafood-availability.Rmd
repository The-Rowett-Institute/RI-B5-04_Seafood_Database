...
# Remove products not included in dietary recommendations  e.g. algae, fish meal, "fish oil", "Seaweed and other algae",
...
ggplot(df_UK_seafoodDB, aes(fill = Commodity, y = Value, x = SpeciesType)) + 
  geom_bar(position='stack', stat='identity') + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

ggplot(df_UK_seafoodDB, aes(x = SpeciesType, y = Value, fill = Commodity)) +
  geom_col(position = "dodge") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

# Create stacked bar plot displaying weight for each SACN, by commodity. 
ggplot(df_UK_seafoodDB, aes(fill = Commodity, y = Value, x = SACN)) + 
  geom_bar(position='stack', stat='identity') + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
annual_seafood <- aggregate(supplies ~ Year, data = df_UK_seafoodDB_short_clean, sum)

annual_seafoods <- plot(supplies ~ Year, data = annual_seafood, pch= 20, type = "l", cex.axis = 1.0, cex.lab = 1.5, ylab = "Seafood available", xlab = "Year")
