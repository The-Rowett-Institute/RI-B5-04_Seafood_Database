...
## Calculate individual seafood commodities supplied to the UK between 2009 and 2019 

```{r Calculate average seafood supplied to the UK by species}
# Separate imports and export data
seafoodImports <- subset(df_trade_HMRC_data, Commodity == "Imports")
seafoodExports <- subset(df_trade_HMRC_data, Commodity == "Exports")
...
colnames(df_UKseafood_200918) <- c("SpeciesNEW", "Year", "ValueCapture", "ValueAqua", "ValueImport", "ValueExport")

# Convert NA to zeros (otherwise availability cannot be calculated)  
df_UKseafood_200918[is.na(df_UKseafood_200918)] <- 0

# Create new column to calculate availability for each species
df_UKseafood_200918$available <- (df_UKseafood_200918$ValueCapture + df_UKseafood_200918$ValueAqua + df_UKseafood_200918$ValueImport) - df_UKseafood_200918$ValueExport
```


## Figure 8. Boxplot of variability in individual seafood commodities supplied to the UK between 2009 and 2018

```{r Plot boxplot of annual variability of individual seafood commodities supplied to the UK}
...
boxplot(available ~ SpeciesNEW, data = order_seafood, col = "lightgray", 
        ylab = "Seafood supplied (g/capita/week)", xlab = "", las = 3)
...
bymedian <- with(df_UKseafood_200918, reorder(SpeciesNEW, -available, median))

# Plot re-order boxplot from most to least available seafood supplied
boxplot(available ~ bymedian, data = df_UKseafood_200918, ylab= "seafood availability
        (g/capita/week)", xlab= "", las= 2, col= "lightgrey")
