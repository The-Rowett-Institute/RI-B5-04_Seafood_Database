...
```{r Load the database}
# Load database 
df_UK_seafoodDB <- vroom(file=paste(filepath,"Seafood-database_preliminary.csv", sep="/"), na = c(".."))

# Check the structure 
str(df_UK_seafoodDB)

# Set value to numerical 
df_UK_seafoodDB$Value <- as.numeric(df_UK_seafoodDB$Value)
```

```{r Final clean}
# Remove products for non-human consumption 

# Remove entries not included in dietary recs e.g. aglae, fishmeal 
...
```{r}
seafoodCommodity <- as.data.frame(unstack(df_UK_seafoodDB, Commodity ~ Year))
