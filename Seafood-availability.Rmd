...
UK seafood supplies between 2009 and 2019 were derived from data on imports and production (from both capture fisheries and aquaculture), accounting for exports. UK fish dietary recommendations were compared with national seafood supplies. 

The data set created from the RESAS B5 project was used to calculate UK seafood availability. 
Seafood production (aquaculture, capture), trade (imports and exports), consumption and purchasing data were collated from separate data sources. Appropriate conversion factors were previously applied (live weight to processed weight). Data are provided in grams. grams/capita/day.
...
df_captProd_NS_data <- vroom(file=paste(filepath,"ProductionData_NS_clean-edited.csv", sep="/"))
...
df_aquaProd_Eurostat_data <- vroom(file=paste(filepath,"ProcessedDataFarmedData_Eurostat_clean.csv", sep="/"), na = c("NA"))
...
```{r Load UK trade data}
# Read in UK trade data (further cleaned)
df_trade_HMRC_data <- vroom(file=paste(filepath,"ProcessedDataTrade_HMRC_clean.csv", sep="/"))
md.pattern(df_trade_HMRC_data, rotate.names =  TRUE)
```


```{r Load UK fish purchases}
...
# Check coulmn names match
colnames(df_captProd_NS_data)
colnames(df_aquaProd_Eurostat_data)
...
# Identify unique fish species
unique(UK_seafood_data$SpeciesNEW)
