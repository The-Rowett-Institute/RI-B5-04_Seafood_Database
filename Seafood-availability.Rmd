...
df_captProd_NS_data <- vroom(file=paste(filepath,"CaptureProduction_NS_edit_clean.csv", sep="/"))
...
seafood <- plot(available ~ Year, data = df_avgSeafood, pch= 20, type = "l", cex.axis = 1.0, cex.lab = 1.5, ylab = "Seafood available (g/capita/week)", xlab = "Year", ylim = c(0, 300))
...
plot(Aquaprod ~ Year, data = df_avgSeafood, cex.axis = 1.0, cex.lab = 1.5, ylab = "UK seafood production (g/capita/week)", xlab = "Year", ylim = c(0, 100))
lines(CaptureProd ~ Year, data = df_avgSeafood)

# Separate figures
plot(CaptureProd ~ Year, data = df_avgSeafood, cex.axis = 1.0, cex.lab = 1.5, ylab = "UK seafood production (g/capita/week)", xlab = "Year", ylim = c(0, 100))
lines(df_avgSeafood$CaptureProd)

plot(Aquaprod ~ Year, data = df_avgSeafood, cex.axis = 1.0, cex.lab = 1.5, ylab = "UK seafood production (g/capita/week)", xlab = "Year", ylim = c(0, 100), lines(df_avgSeafood$CaptureProd))

```



```{r Figure 2. UK average net seafood availabilty between 2009 and 2019}

# Sum amount of fish available g/capita/week per year for each commodity. 

# Name columns in aggregate function
captProd <- aggregate(list(CaptureProd = df_captProd_NS_data$ValueGramsCapitaWeek), 
                      list(Year = df_captProd_NS_data$Year),
                      list(SCANType = df_captProd_NS_data$SACN), sum)

captProd <- aggregate(list(CaptureProd = df_captProd_NS_data$SACN), 
                      list(Year = df_captProd_NS_data$Year), sum)

captProd <- aggregate(df_captProd_NS_data$ValueGramsCapitaWeek, 
                                      by = list(df_captProd_NS_data$SACN), mean)

```


## Figure 3. Mapping SACN types (oily/lean/shellfish) per commodity
...
```{r Merge commodity data bweteen 2009 and 2018 }
# Merge aggregated data
...
```{r Idenitfying the most produced species }
df_captProd_NS_data
...
arrange(capture13, desc(ValueGramsCapitaWeek)) # mackerel, haddock, herring, scallops, crabs # change
...
### Figure 4. Average seafood availabilty between 2009 and 2018

```{r Calculate average UK seafood supply (g/capita/week) between 2009 and 2018}
# Calculate average amount of fish available g/capita/week per year for each commodity between 2009 and 2018. 

captProd_avg <- mean(df_captProd_NS_data$ValueGramsCapitaWeek) 
captProd_avg <- mean(df_captProd_NS_data$ValueGramsCapitaWeek) 
captProd_avg <- mean(df_captProd_NS_data$ValueGramsCapitaWeek) 
captProd_avg <- mean(df_captProd_NS_data$ValueGramsCapitaWeek) 
...
aquaProd <- aggregate(list(AquaProd = df_aquaProd_Eurostat_data_omit$ValueGramsCapitaWeek), 
                      list(Year = df_aquaProd_Eurostat_data_omit$Year), mean)
aquaProd <- subset(aquaProd, Year < 2019)# up to 2018

householdPurch <- aggregate(list(HPurch = df_housePurch_DEFRA_data$ValueGramsCapitaWeek), 
                      list(Year = df_housePurch_DEFRA_data$Year), mean)
householdPurch <- subset(householdPurch, Year < 2019) # up to 2018

eatenOutPurch <- aggregate(list(EOPurch = df_eatenOutPurch_DEFRA_data$ValueGramsCapitaWeek), 
                      list(Year = df_eatenOutPurch_DEFRA_data$Year), mean)
eatenOutPurch<- subset(eatenOutPurch, Year < 2019) # up to 2018
...
imports <- aggregate(list(Imports = seafoodImports$ValueGramsCapitaWeek), 
                      list(Year = seafoodImports$Year), mean)
imports <- subset(imports, Year < 2019) # up to 2018

exports <- aggregate(list(Exports = seafoodExports$ValueGramsCapitaWeek), 
                      list(Year = seafoodExports$Year), mean)
exports <- subset(exports, Year < 2019) # up to 2018
...
```{r Merge average commodities bweteen 2009 and 2018 }
# Merge aggregated data
df_avgSeafood <- cbind(captProd, aquaProd[,2], imports[,2], exports[,2] )

# Rename column in the data frame
names(df_avgSeafood) <- c("Year", "CaptureProd", "Aquaprod", "Imports", "Exports")
colMeans(df_avgSeafood)
