...
```{r Final clean}
# Remove products for non-human consumption 

# Remove products not included in dietary recommendations  e.g. algae, fish meal, fish oil
...
```{r Exploratory plots}
# Create stacked bar plot displaying  value for each species, stacked by commodity. 
ggplot(df_UK_seafoodDB, aes(fill = Commodity, y = Value, x = Species)) + 
  geom_bar(position='stack', stat='identity')

# Plot bars side by side
ggplot(df_UK_seafoodDB, aes(x = Species, y = Value, fill = Commodity)) +
  geom_col(position = "dodge")
```


```{r}
# Separate out other "species"
df_UK_seafood_prod <- subset(df_UK_seafoodDB, Commodity == "Production")
...
Unstack the data frame to create a new column calculating seafood availability per species 


```{r Unstack the data frame}
str(df_UK_seafoodDB)

library(reshape2)
library(reshape)

df_UK_seafoodDB_short <- cast(df_UK_seafoodDB, Species  ~ Commodity)
 
unique(df_UK_seafoodDB$Commodity)
```
...
```{r Unstack the data frame}
# Subset commodities and create new dataset
df_UK_seafood_prod <- subset(df_UK_seafoodDB, Commodity == "Production")
df_UK_seafood_imp <- subset(df_UK_seafoodDB, Commodity == "Imports")
df_UK_seafood_exp <- subset(df_UK_seafoodDB, Commodity == "Exports")




# Merge data sets by species and year 
df_UK_seafoodDB_short <- merge(df_UK_seafood_prod, df_UK_seafood_imp, by = c("Species", "Year")), all= TRUE)

inner.joi

str(df_UK_seafood_prod)
str(df_UK_seafood_imp)


df_UK_seafoodDB_short <- merge(df_UK_seafood_prod, df_UK_seafood_imp, by = "Species", all= TRUE)

df_UK_seafood_prod <- df_UK_seafood_prod %>% full_join(., df_UK_seafood_imp, by = c('Species' = 'Species', "Year"= "Year"))
df_UK_seafoodDB_short <- df_UK_seafood_prod %>% full_join(., df_UK_seafood_exp, by = c('Species' = 'Species', "Year"= "Year"))


# Create new data frame with desired variables 
colnames(df_UK_seafoodDB_short)

# Select desired variables 
df_UK_seafoodDB_short_clean <- df_UK_seafoodDB_short[c("Species", "SpeciesType", "SACN.x", "Value.x", "Value.y", "Value", "Year")]

# Rename variables
names(df_UK_seafoodDB_short_clean) <- c("Species", "SpeciesType", "SACN", "Prod", "Imp", "Exports", "Year")

# Calculate seafood supplies
df_UK_seafoodDB_short_clean$supplies <- (df_UK_seafoodDB_short_clean$Prod + df_UK_seafoodDB_short_clean$Imp) - df_UK_seafoodDB_short_clean$Exports

# Create box plot of supplies
boxplot(supplies ~ Species, data = df_UK_seafoodDB_short_clean, col = "lightgray", 
