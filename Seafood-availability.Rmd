...
annual_seafood <- plot(available ~ Year, data = df_annualSeafood, pch= 20, type = "l", cex.axis = 1.0, cex.lab = 1.5, ylab = "Seafood available", xlab = "Year", ylim = c(0, 300))
...
df_UK_seafoodProduction <- df_captureProd_NS_clean_data[,2:4] %>% 
...
str(df_UK_seafoodSupply_new)

mackerel  <- subset(df_UK_seafoodSupply_new, RevisedMCS == "Mackerel")

boxplot(Available ~ RevisedMCS, data = mackerel, col = "lightgray", 
        ylab = "Seafood supplied (T)", xlab = "", las = 3, cex.axis = 1.0)
