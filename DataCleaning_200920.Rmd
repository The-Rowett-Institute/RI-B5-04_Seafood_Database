---
title: "RI-B5-04: UK seafood supply chains. Data cleaning."
...
This R markdown document outlines how the data compiled in the UK seafood database were cleaned. Data on seafood production (both capture and aquaculture), trade, consumption and purchasing behavior between 2009 and 2020 were sourced. 
...
MCS_translations <- vroom(file=paste(filepath,"AllMainCommercialSpecies_translations_updated.csv", sep="/"))
...
```{r Combine data sets: i) 2008 to 2011; ii) 2012 to 2016; and iii) 2017 to 2019)}
# Combine capture production data i) 2009 to 2011; ii) 2012 to 2016; and iii) 2017 to 2020)
df_UKV_LandingUKAbroad_200918_long <- rbind(df_UKV_LandingUKAbroad_201720_long,
...
# Join data frame with MCS data frame by species name
df_UKV_LandingUKonly_200920_long <- df_UKV_LandingUKonly_200920_long %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
```{r Apply conversion factors to calculate edible portion}
# Identify unique conversion factors in the data set
unique(df_UKV_LandingUKonly_200920_long$CF_value) # no processed data here so no 1 Volumes

# Create new column and apply conversion factors Volumes
df_UKV_LandingUKonly_200920_long$apply_CF <- df_UKV_LandingUKonly_200920_long$Volume * df_UKV_LandingUKonly_200920_long$CF_value
...
# Select desired columns for cleaned data set-- 16  variables
df_UKV_LandingUKonly_200920_clean <- df_UKV_LandingUKonly_200920_long[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "Volume", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]

# Check column names
colnames(df_UKV_LandingUKonly_200920_clean)

# Rename "SeafoodSpecies" column 
#df_UKV_LandingUKAbroad_200918_cleaned <- rename(df_UKV_LandingUKAbroad_200918_clean, c("SeafoodSpecies" = "Species"))

df_UKV_LandingUKonly_200920_cleaned <- df_UKV_LandingUKonly_200920_clean %>% rename(Species = SeafoodSpecies)
...
# Read in .csv file 
df_EUROSTAT_UK_aquaculture_data <- vroom(file=paste(filepath,"EUROSTAT_UK_aquaculture-200920.csv", sep="/"), na = c("NA"))
...
df_UKFarmed_200920 <- melt(df_EUROSTAT_UK_aquaculture_data, id.vars = c("Species"), variable.name = "Year", value.name = "Volume")
...
df_UKFarmed_200920 <- df_UKFarmed_200920 %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
...
# Identify unique conversion factors in the data set
unique(df_UKFarmed_200920$CF_value) # no processed data here so no 1 Volumes

# Create new column and apply conversion factors Volumes
df_UKFarmed_200920$apply_CF <- df_UKFarmed_200920$Volume * df_UKFarmed_200920$CF_value
...
```{r Add additional variables to data set}
# Create "cleaned" data frame, defining units and data source 
df_UKFarmed_200920$DataSupplier <- "Eurostat"
...
df_UK_aquaculture_200920 <- df_UKFarmed_200920[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "Volume", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
df_UK_aquaculture_200920_cleaned <- df_UK_aquaculture_200920 %>% rename(Species = SeafoodSpecies)
...
```{r Check dataset}
# Check for duplicates
duplicates_Eurostat <- df_UK_aquaculture_200920_cleaned %>% select(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% group_by(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% 
  filter(n()>1) %>% mutate(n = n())

# Check data transforms from long and thin to short and wide
shape_Eurostat <- df_UK_aquaculture_200920_cleaned %>% 
  select(DataSupplier, DataSet, Commodity, Species, SACN, Year, Volume, Units) %>% 
  filter(Commodity %in% c("Production")) %>% 
  pivot_wider(names_from = Commodity, values_from = Volume)
...
```{r}
#Age
#change age to groups in Ind1to4
AgeGrp4_1to4 <- rep(0,length(Ind_Data_1to4$age))
for(i in 1:length(AgeGrp4_1to4)){
  if(Ind_Data_1to4$age[i]<0){
    AgeGrp4_1to4[i] <- NA
    }
  else{
    if(Ind_Data_1to4$age[i]<=10){
      AgeGrp4_1to4[i] <- 1
    }
    else{
      if(Ind_Data_1to4$age[i]<=18){
        AgeGrp4_1to4[i] <- 2
      }
      else{
        if(Ind_Data_1to4$age[i]<=64){
          AgeGrp4_1to4[i] <- 3
        }
        else{
          AgeGrp4_1to4[i] <- 4
...
# Change age to groups in Ind5to6
AgeGrp4_5to6 <- rep(0,length(Ind_Data_5to6$age))
for(i in 1:length(AgeGrp4_5to6)){
  if(Ind_Data_5to6$age[i]<0){
    AgeGrp4_5to6[i] <- NA
  }
  else{
    if(Ind_Data_5to6$age[i]<=10){
      AgeGrp4_5to6[i] <- 1
    }
    else{
      if(Ind_Data_5to6$age[i]<=18){
        AgeGrp4_5to6[i] <- 2
      }
      else{
        if(Ind_Data_5to6$age[i]<=64){
          AgeGrp4_5to6[i] <- 3
        }
        else{
          AgeGrp4_5to6[i] <- 4
...
#change age to groups in Ind7to8
AgeGrp4_7to8 <- rep(0,length(Ind_Data_7to8$age))
for(i in 1:length(AgeGrp4_7to8)){
  if(Ind_Data_7to8$age[i]<0){
    AgeGrp4_7to8[i] <- NA
  }
  else{
    if(Ind_Data_7to8$age[i]<=10){
      AgeGrp4_7to8[i] <- 1
    }
    else{
      if(Ind_Data_7to8$age[i]<=18){
        AgeGrp4_7to8[i] <- 2
      }
      else{
        if(Ind_Data_7to8$age[i]<=64){
          AgeGrp4_7to8[i] <- 3
        }
        else{
          AgeGrp4_7to8[i] <- 4
...
#change age to groups in Ind9to11
AgeGrp4_9to11 <- rep(0,length(Ind_Data_9to11$AgeR))
for(i in 1:length(AgeGrp4_9to11)){
  if(Ind_Data_9to11$AgeR[i]<0){
    AgeGrp4_9to11[i] <- NA
  }
  else{
    if(Ind_Data_9to11$AgeR[i]<=10){
      AgeGrp4_9to11[i] <- 1
    }
    else{
      if(Ind_Data_9to11$AgeR[i]<=18){
        AgeGrp4_9to11[i] <- 2
      }
      else{
        if(Ind_Data_9to11$AgeR[i]<=64){
          AgeGrp4_9to11[i] <- 3
        }
        else{
          AgeGrp4_9to11[i] <- 4
...
age <- c(AgeGrp4_1to4, AgeGrp4_5to6, AgeGrp4_7to8, AgeGrp4_9to11, Ind_Data_12$AgeGrp4)
```


```{r Join data frames}
NDNS_UK_Yr1_12_data <- rbind(df_NDNS_foodlevel_yr1_4_data,
                            df_NDNS_foodlevel_yr5_6_data,
                            df_NDNS_foodlevel_yr7_8_data,
                            df_NDNS_foodlevel_yr9data, 
                            df_NDNS_foodlevel_yr10_data,
...
# Join data frame with MCS data frame by species name
df_DEFRA_purchase_201620_data <- df_DEFRA_purchase_201620_data %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
# Join data frame with MCS data frame by species name
df_DEFRA_household_200920 <- df_DEFRA_household_200920 %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
df_DEFRA_household_200920_agg <- aggregate(Volume ~ SeafoodSpecies + Year + SpeciesType + SACN, data =
...
# Select desired columns for cleaned data set
df_DEFRA_household_200920_cleaned <- df_DEFRA_household_200920_aggYear[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "totUK", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
# Join data frame with MCS data frame by species name
df_DEFRA_eatenOut_200920 <- df_DEFRA_eatenOut_200920 %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
```{r Seafood consumption (g/capita/week), by species}
# Sum seafood consumption per person per species
df_DEFRA_eatenOut_200920_agg <- aggregate(Volume ~ SeafoodSpecies + Year + SpeciesType + SACN, data =
...
# Select desired columns for cleaned data set
df_DEFRA_eatenOut_200920_cleaned <- df_DEFRA_eatenOut_200920_aggYear[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "totUK", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
