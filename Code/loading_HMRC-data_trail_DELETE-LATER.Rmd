...
=======
<<<<<<< HEAD
>>>>>>> 28b1a03ef6dce92bcdfeb50e123827eeb64650a6
df_test <- unique(df_EUMOFA_CN8[,c('CN-8','CN-8 product name',"CF")])
df_test$Loop_Help <- gsub(r"{\s*\(excl.[^\)]+\)}","",as.character(df_test$`CN-8 product name`)) # removing the "excluding"-comment in the EUMOFA Product name to prevent finding our fish types in them and wrongfully assigning them to the CN-8
<<<<<<< HEAD
||||||| merged common ancestors
...
=======
df_test <- unique(df_EUMOFA_CN8[,c('CN-8','CN-8 product name')])
>>>>>>> 028e256040e0a631fbcc5e58de49251975a4e3e1
>>>>>>> 28b1a03ef6dce92bcdfeb50e123827eeb64650a6
df_test$FishType <- NA
df_test$Species  <- NA
df_test$Hits <- 0 # to check whether a row had been assign a fish type / species more then once. If the number > 1. It will need to be double checked   
...
# Current problem is. That is will also find fish species that are "excluded" 
for(i in 1:length(df_test$`CN-8 product name`)){
  for(f in 1:length(df_FishType_Species$Species)){
    if(grepl(df_FishType_Species$Species[f],df_test$Loop_Help[i],ignore.case = TRUE)){
      df_test$Species[i] <- df_FishType_Species$Species[f]
      df_test$FishType[i] <-  df_FishType_Species$FishType[f]
      df_test$Hits[i] <- df_test$Hits[i] + 1 # this will count up
      }else{
        if(is.na(df_test$Species[i])){
          df_test$Species[i] <- "Not in list"
          df_test$FishType[i] <-"Not in list"
          }
      }
    }
  }

print(paste(sum(df_test$Hits>1 & df_test$CF>=1), "rows need to be double checked. As they had more than 1 fish species assign and are for human consumpation (EUMOFA CF > 0"))

print(paste(sum(grepl("Not in list",df_test$Species) & df_test$CF>0), "rows need to be double checked. As they are for human consumpation (EUMOFA CF > 0), but not on our list for fish species"))
