...
df_HRMC.sum <- df_HMRC_4DB %>% group_by(Species = RevisedMCS, SpeciesType = SpeciesType_AL, Year, Commodity) %>% summarise(`Net Mass (g)`= sum(`Net Mass (kg)`)*1000,`Net Mass (kg)`= sum(`Net Mass (kg)`))
