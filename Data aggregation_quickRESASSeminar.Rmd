...
MCS_translations <- vroom(file=paste(filepath,"AllMainCommercialSpecies_translations_checked_again_again.csv", sep="/"))
...
df_UKV_LandingUKonly_200918_clean <- df_UKV_LandingUKonly_200918_long[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SACN", "Volume", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
dulpicates_MMO <- df_UKV_LandingUKonly_200918_cleaned %>% select(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% group_by(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% 
  filter(n()>1) %>% mutate(n = n())

# Check data transforms from long and thin to short and wide
shape_MMO <- df_UKV_LandingUKonly_200918_cleaned %>% 
...
# Stating row.names = FALSE removes the initial column of the new .csv file where the rows are numbered
write.csv(df_UKV_LandingUKonly_200918_cleaned, file = paste(filepath,"MMO_OnlyUKVessels_LandingUK_PreliminaryCleaned_Now.csv", sep = ""), row.names = FALSE)
...
# Read in .csv file 
df_EUROSTAT_UK_aquaculture_data <- vroom(file=paste(filepath,"EUROSTAT_UK_aquaculture-200920.csv", sep="/"), na = c(":"))
...
```{r Aggregate volumes per species}
# Aggregate species. Rainbow trout entered twice (for salt and freshwater)
#colnames(df_UKFarmed_200918)
#df_UKFarmed_200918_check <- aggregate(SeafoodSpecies ~ Volume + Year + SACN + CF_classification, data = df_UKFarmed_200918, sum)
...
df_UK_aquaculture_200918 <- df_UKFarmed_200918[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SACN", "Volume", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
```{r Check dataset}
# Check for duplicates
duplicates_Eurostat <- df_UK_aquaculture_200918_cleaned %>% select(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% group_by(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% 
  filter(n()>1) %>% mutate(n = n())

# Check data transforms from long and thin to short and wide
shape_Eurostat <- df_UK_aquaculture_200918_cleaned %>% 
  select(DataSupplier, DataSet, Commodity, Species, SACN, Year, Volume, Units) %>% 
  filter(Commodity %in% c("Production")) %>% 
  pivot_wider(names_from = Commodity, values_from = Volume)
...
write.csv(df_UK_aquaculture_200918_cleaned, file = 
            paste(filepath,"CEFAS_AquacultureData_PreliminaryCleaned_NOW.csv", sep = ""), row.names = FALSE)
...
```{r Check dataset}
# Check for duplicates
duplicates_DEFRAHouse <- df_DEFRA_household_200918_aggYear_cleaned %>% select(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% group_by(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% 
  filter(n()>1) %>% mutate(n = n())

# Check data transforms from long and thin to short and wide
shape_DEFRAHouse <- df_DEFRA_household_200918_aggYear_cleaned %>% 
  select(DataSupplier, DataSet, Commodity, Species, SACN, Year, Volume, Units) %>% 
...
```{r Check dataset}
# Check for duplicates
duplicates_DEFRAOut <- df_DEFRA_eatenOut_200918_cleaned %>% select(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% group_by(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% 
  filter(n()>1) %>% mutate(n = n())

# Check data transforms from long and thin to short and wide
shape_DEFRAOut <- df_DEFRA_eatenOut_200918_cleaned %>% 
  select(DataSupplier, DataSet, Commodity, Species, SACN, Year, Volume, Units) %>% 
  filter(Commodity %in% c("Purchases")) %>% 
  pivot_wider(names_from = Commodity, values_from = Volume)
