...
theme_set(theme_bw()%+replace%
            theme(
              ...
              axis.text = element_text(size = 24, colour = "gray40"), # increase to make readable
...
df_UK_supplies_tot <- aggregate(Supplies_capWeek ~ Year, data = df_UK_seafoodDB_PCW, sum)
...
# Select variables 
df_UK_seafoodCapita_suppCons <- df_UK_seafoodDB_capita[c("Supplies_capitaWeek", "Consumption_capWeek", "Year", "Species", "SpeciesType", "SACN")]
...
# Sum annual seafood supplies per capita 
df_UK_supplies_tot <- aggregate(Supplies_capWeek ~ Year, data = df_UK_seafoodDB_PCW, sum)
df_UK_consummed_tot <- aggregate(Consumption_capWeek ~ Year, data = df_UK_seafoodDB_PCW, sum)

str(df_UK_seafoodDB_PCW)
