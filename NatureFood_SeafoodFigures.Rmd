...
# Subset supply
df_nutrientSupply <- df_UK_seafoodDBNC_2018 %>% select(Species, Year, SACN,
                                                  SupplyPUFA_gPCW,SupplyVitaminA_mgPCW, 
                                         SupplyVitaminD_mgPCW, SupplyVitaminB12_mgPCW,
                                         SupplyIron_mgPCW, SupplyCalcium_mgPCW,   
                                         SupplySelenium_mgPCW, SupplyIodine_mgPCW)
                                         
# Subset exports
df_nutrientExports <- df_UK_seafoodDBNC_2018 %>% select(Species, Year, SACN,
                                                   ExportsPUFA_gPCW,ExportsVitaminA_mgPCW,
                                         ExportsVitaminD_mgPCW, ExportsVitaminB12_mgPCW,
                                         ExportsIron_mgPCW, ExportsCalcium_mgPCW,
                                         ExportsSelenium_mgPCW, ExportsIodine_mgPCW)



# Restructure data set
df_UK_nutrientExports <- melt(df_nutrientExports, id.vars = c("Year", "Species", "SACN"), variable.name = "Nutrient", value.name = "Volume")


# Only include nutrients if volume is greater than 0.
df_UK_nutrientExports_values <- subset(df_UK_nutrientExports, Volume > 0)

# Plot amount of nutrients exported (g/capita/week) in 2018 
ggplot(df_UK_nutrientExports_values, aes(Species, Volume, fill= Nutrient)) +
  geom_bar(position = "dodge", stat= "identity", color = "black") +
  scale_y_continuous(name = "Amount of nutrient exported (capita/day)", limits = c(0, 12)) + # set axis limits
