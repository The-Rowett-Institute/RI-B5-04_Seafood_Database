...
```{r Seafood supplies consumption and purchases between 2009 and 2018- Y-axis title removed }
# Convert to a factor
df_UK_CPS_total$Commodity_f <-  factor(df_UK_CPS_total$Commodity, levels = c("Supply", "Purchases", "Consumption"))
...
```{r Calculate seafood wasted at the household level (purchases less consumption)}
df_UK_seafoodDB_PCW_short
str(df_UK_seafoodDB_PCW_short)

df_UK_seafoodWaste <- df_UK_seafoodDB_PCW_short$Consumption %>% select(Year, Purchases, Consumption)

aggregate(Volume ~ Commodity + Year + Species + SpeciesType + SACN, data = df_UK_seafoodDB_PCW, sum)
...
```{r Domestic seafood production- remove y axis}
# Plot average seafood supply (g/capita/week), to SACN level (lean and oily) between 2009 and 2018. 
ggplot(df_UK_seafoodProduction_LOSO, aes(y = Production, x = Year, fill = SACN_f)) +
  geom_bar(position = "stack", stat = "identity", color = "black", width = 0.5) + 
  scale_y_continuous(name = "NA", limits = c(0,300)) +
  scale_x_continuous(name = "Year", breaks = seq(2009, 2018, 1), 
                     guide = guide_axis(angle = 45)) +
  scale_fill_manual(values = c("#999999", "#DDCC77",  "#CC79A7","#D55E00")) +
  geom_hline(yintercept = 280, linetype = "dashed", color = "red", linewidth = 2.0) + 
