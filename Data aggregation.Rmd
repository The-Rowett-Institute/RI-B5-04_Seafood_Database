...
# Read in .csv file
UK_population_200918 <- vroom(file=paste(filepath,"UKPopulation_ONS_PreliminaryCleaned.csv", sep="/"), na = c(".."))
...
df_UK_AllUKLandings_201718_long <- melt(df_UK_AllUKLandings_201718_data, id.vars = c("Species"), variable.name = "Year", value.name = "Value")
```
...
# Change structure of the data set 
df_UK_AllUKLandings_201216_long <- melt(df_UK_AllUKLandings_201216_data, id.vars = c("Species"), variable.name = "Year", value.name = "Value")
```
...
# Change structure of the data set 
df_UK_AllUKLandings_200911_long <- melt(df_UK_AllUKLandings_200911_data, id.vars = c("Species"), variable.name = "Year", value.name = "Value")
...
df_UK_AllUKLandings_200918$SpeciesNEW <- df_UK_AllUKLandings_200918$Species

# Join data frame with MCS data frame by species name
df_UK_AllUKLandings_200918 <- df_UK_AllUKLandings_200918 %>% inner_join(., MCS_translations, by = c('SpeciesNEW' = 'MCS (DO NOT TOUCH)'))
...
unique(df_UK_AllUKLandings_200918$CF_value) # no processed data here so no 1 values

# Create new column and apply conversion factors values
df_UK_AllUKLandings_200918$apply_CF <- df_UK_AllUKLandings_200918$Value * df_UK_AllUKLandings_200918$CF_value
...
df_UK_AllUKLandings_200918$Value <- (df_UK_AllUKLandings_200918$apply_CF * 1000) * 1000000
```
...
df_UK_AllUKLandings_200918_clean <- df_UK_AllUKLandings_200918[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
write.csv(df_UK_AllUKLandings_200918_clean, file = paste(filepath,"AllLandingsintoUK_MMO_PreliminaryCleaned.csv", sep = ""), row.names = FALSE)
...
df_UKFarmed_200918 <- melt(df_EUROSTAT_UK_aquaculture_data, id.vars = c("Species"), variable.name = "Year", value.name = "Value")
...
df_UKFarmed_200918$SpeciesNEW <- df_UKFarmed_200918$Species

# Merge data frame with MCS data frame by species name
df_UKFarmed_200918 <- df_UKFarmed_200918 %>% inner_join(., MCS_translations, by = c('SpeciesNEW' = 'MCS (DO NOT TOUCH)'))
...
# Identify unique conversion factors in the data set
unique(df_UKFarmed_200918$CF_value) # no processed data here so no 1 values

# Create new column and apply conversion factors values
df_UKFarmed_200918$apply_CF <- df_UKFarmed_200918$Value * df_UKFarmed_200918$CF_value
...
df_UKFarmed_200918$Value <- df_UKFarmed_200918$apply_CF * 1000000
...
df_UK_aquaculture_200918 <- df_UKFarmed_200918[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
filepath <- paste(data_dir,"ProcessedData", sep = "")
write.csv(df_UK_aquaculture_200918, file = 
...
# Select desired columns for cleaned data set
df_cleaned_UK_HMRC_200919 <- df_trade_HMRC_data[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
filepath <- paste(data_dir,"ProcessedData", sep = "")
write.csv(df_cleaned_UK_HMRC_200919, file = paste(filepath,"Trade_HMRC_clean-edit.csv", sep = ""), row.names = FALSE)
...
# Load NDNS yrs 1 - 4
df_NDNS_foodlevel_yr1_4_data <- vroom(file= paste(filepath,"NDNS_UK_FoodLevel-Y1-4_short.csv", sep = "/"))

# Load NDNS yrs 5 - 6 
df_NDNS_foodlevel_yr5_6_data <-
  vroom(file = paste(filepath,"NDNS_UK_FoodLevel-Y5-6_short.csv", sep = "/"))

# Load NDNS yrs 7 - 8 
df_NDNS_foodlevel_yr7_8_data <-
  vroom(file = paste(filepath,"NDNS_UK_FoodLevel-Y7-8_short.csv", sep = "/"))

# Load NDNS yrs 9
df_NDNS_foodlevel_yr9data <- 
  vroom(file = paste(filepath,"NDNS_UK_FoodLevel-Y9_short.csv", sep = "/"))

# Load NDNS yrs 10
df_NDNS_foodlevel_yr10_data <- 
  vroom(file = paste(filepath,"NDNS_UK_FoodLevel-Y10_short.csv", sep = "/"))

# Load NDNS yrs 11
df_NDNS_foodlevel_yr11_data <- 
  vroom(file = paste(filepath,"NDNS_UK_FoodLevel-Y11_short.csv", sep = "/"))
...
colnames(df_NDNS_foodlevel_yr9data)
colnames(df_NDNS_foodlevel_yr10_data)
colnames(df_NDNS_foodlevel_yr11_data)

# Check if columns match up
all_equal(df_NDNS_foodlevel_yr5_6_data, df_NDNS_foodlevel_yr9data)


# Rename "AgeR"
#colnames(df_NDNS_foodlevel_yr9_11_data)[colnames(df_NDNS_foodlevel_yr9_11_data) == 'AgeR'] <- 'Age'
...
Now all species are in a separate column. Some species are duplicated per year i.e. salmon fillet/ salmon pate/ smoked salmon now all called salmon so need to aggregate (add) salmon consumption each reported year.  

```{r Sum seafood consumption per person per species}
# Sum seafood consumption per person per species
df_NDNS_seafood_aggregate_person_Yr1_11 <- aggregate(totfishperprod ~ Species + SurveyYear + seriali, data =
                                                df_NDNS_seafood_consumption_Yr1_11, sum)

```


```{r Calculate the average amount reportedly consumed per day}
# Calculate daily fish consumption
df_NDNS_seafood_aggregate_person_Yr1_11$gDay <- df_NDNS_seafood_aggregate_person_Yr1_11$totfishperprod/ 4

```


```{r Sum seafood consumption per species per year}
# Sum seafood consumption per person per species
df_NDNS_seafood_aggregate_Yr1_11 <- aggregate(gDay ~ Species + SurveyYear, data =
                                                df_NDNS_seafood_aggregate_person_Yr1_11, mean)
...
```{r Aggregate seafood species per year}
# Aggregate grams by species and year 
df_NDNS_seafood_aggregate_Yr1_11 <- aggregate(Value ~ Species + SurveyYear, data =
                                                df_NDNS_seafood_consumption_Yr1_11, sum)

```


All cleaned data sets should be in the same units. Convert grams/capita/week to grams/UK/year


```{r Convert grams/capita/week to grams/UK/year}
df_NDNS_seafood_aggregate_Yr1_11$Value
```
...
SurveyYear <- unique(df_NDNS_seafood_consumption_Yr1_11$SurveyYear)
Year <- c(200809, 200910, 201011, 201112, 201213, 201314, 201415, 201516, 201617, 201718, 201819)  
...
df_NDNS_seafood_consumption_Yr1_11_num <- merge(df_NDNS_seafood_consumption_Yr1_11, df_NDNSYearData, by = "SurveyYear")
...
```{r Add additional variables to data set}
# Create "cleaned" data frame, defining units and data source 
df_NDNS_seafood_consumption_Yr1_11_num$DataSupplier <- "NDNS"
df_NDNS_seafood_consumption_Yr1_11_num$DataSet <- "FoodDiaries"
df_NDNS_seafood_consumption_Yr1_11_num$Commodity <- "Consumption"
df_NDNS_seafood_consumption_Yr1_11_num$Flag <- "ConV2"
df_NDNS_seafood_consumption_Yr1_11_num$TemporalResolution <- "FinancialYear"
df_NDNS_seafood_consumption_Yr1_11_num$Units <- "Grams/Capita/Week"
df_NDNS_seafood_consumption_Yr1_11_num$FlagDescription <- "Units changed to g/country/year"
...
```{r Select desired variables for the data set}
# Identify columns in data frame
colnames(df_NDNS_seafood_consumption_Yr1_11_num)

# Select desired columns for cleaned data set
df_NDNS_seafood_consumption_Yr1_11_cleaned <- df_NDNS_seafood_consumption_Yr1_11_num[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
# Save cleaned data as .csv
filepath <- paste(data_dir,"ProcessedData", sep = "")
write.csv(df_NDNS_seafood_consumption_Yr1_11_cleaned, file = paste(filepath,"ConsumptionData_NDNS_PreliminaryCleaned.csv", sep = ""), row.names = FALSE)

```



## Purchases (DEFRA family food) UK seafood purchses

Load the UK seafood purchasing data. Purchasing data between 2016 and 2018 is composed of purchases at a household and eaten out level. The two data sets need to be separated. 
...
# Subset years 2015 to 2018
df_DEFRA_purchase_201618_data <- subset(df_DEFRA_purchase_201620_data, Year < 2019)
unique(df_DEFRA_purchase_201618_data$Year)
```


Join seafood purchases data with the seafood translation document before subsetting.

```{r Join data frame with translation document}
# Duplicate species name column (allows us to check species have been renames correctly)
df_DEFRA_purchase_201618_data$SpeciesNEW <- df_DEFRA_purchase_201618_data$codeDESCRIPTION

# Join data frame with MCS data frame by species name
df_DEFRA_purchase_201618_data <- df_DEFRA_purchase_201618_data %>% inner_join(., MCS_translations, by = c('SpeciesNEW' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
### i) Household purchases (2016 to 2018)

```{r Add additional variables to data set}
# Rename estimate column to "Value"
#names(df_DEFRA_household_201618)[names(df_DEFRA_household_201618)=="estimate"] <- "Value"

# Create "cleaned" data frame, defining units and data source 
df_DEFRA_household_201618$DataSupplier <- "DEFRA"
df_DEFRA_household_201618$DataSet <- "FamilyFoods_household"
df_DEFRA_household_201618$Commodity <- "Purchases"
df_DEFRA_household_201618$Flag <- "ConV2"
df_DEFRA_household_201618$TemporalResolution <- "CalendarYear"
df_DEFRA_household_201618$Units <- "Grams/Capita/Week"
df_DEFRA_household_201618$FlagDescription <- "Units changed to g/country/year"
...
```{r Select desired variables for the data set}
# Identify columns in data frame
colnames(df_DEFRA_household_201618)

# Select desired columns for cleaned data set
df_DEFRA_household_201618_cleaned <- df_DEFRA_household_201618[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]

```



### ii) Eating out (2016 to 2018)

```{r Add additional variables to data set}
# Rename estimate column to "Value"
names(df_DEFRA_eatenOut_201618)[names(df_DEFRA_eatenOut_201618)=="estimate"] <- "Value"

# Create "cleaned" data frame, defining units and data source 
df_DEFRA_eatenOut_201618$DataSupplier <- "DEFRA"
df_DEFRA_eatenOut_201618$DataSet <- "FamilyFoods_eatenOut"
df_DEFRA_eatenOut_201618$Commodity <- "Purchases"
df_DEFRA_eatenOut_201618$Flag <- "ConV4, ConV5"
df_DEFRA_eatenOut_201618$TemporalResolution <- "CalendarYear"
df_DEFRA_eatenOut_201618$Units <- "Grams/Capita/Week"
df_DEFRA_eatenOut_201618$FlagDescription <- "Units changed to g/country/year"
...
```{r Select desired variables for the data set}
# Identify columns in data frame
colnames(df_DEFRA_eatenOut_201618)

# Select desired columns for cleaned data set
df_DEFRA_eatenOut_201618_cleaned <- df_DEFRA_eatenOut_201618[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
# Check for missing values
md.pattern(df_DEFRA_household_200915_data, rotate.names = TRUE)
# missing values detected but outside of the desired time frame

# Select only years 2009 to 2015
df_DEFRA_household_200915 <- subset(df_DEFRA_household_200915_data, 
                           select = c("Minor Food Code", "Units", "2009", "2010", "2011", 
                                      "2012", "2013", "2014","2015"))
# Check for missing values
...
# Change structure of the data set 
df_DEFRA_household_200915_long <- melt(df_DEFRA_household_200915, id.vars = c("Minor Food Code", "Units"), variable.name ="Year", value.name = "Value")

```
```{r Join DEFRA household purchaing data i) 2019 to 2015 ii) 2016 to 2018}
# Subset the columns we need 
colnames(df_DEFRA_household_201618)

df_DEFRA_household_201618_to_join <- df_DEFRA_household_201618[c("Year", "codeDESCRIPTION", "estimate")]
...
# Rename variable. Rename "minor foof code"
colnames(df_DEFRA_household_200915_long)
colnames(df_DEFRA_household_201618)
colnames(df_DEFRA_household_201618_to_join)

unique(df_DEFRA_household_200915_long$`Minor Food Code`)

df_DEFRA_household_200918 <- rbind(df_DEFRA_household_201618, df_DEFRA_household_200915_long)
```
...
```{r Join data frame with translation document}
# Duplicate species name column (allows us to check species have been renames correctly)
df_DEFRA_household_200915_long$SpeciesNEW <- df_DEFRA_household_200915_long$`Minor Food Code`

# Join data frame with MCS data frame by species name
df_DEFRA_household_200915_long <- df_DEFRA_household_200915_long %>% inner_join(., MCS_translations, by = c('SpeciesNEW' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
Combine household data sets between 2009 and 2018

```{r Combine household data sets i) 2009 to 2015;  ii) 2016 to 2018}
# Combine household data (2009 to 2015 and 2016 to 2018)
colnames(df_DEFRA_household_200915_long)
colnames(df_DEFRA_household_201618)

# Ensure "Year" column is as a factor value
df_DEFRA_household_200915_cleaned$Year <- as.factor(df_DEFRA_household_200915_cleaned$Year)
df_DEFRA_household_201618_cleaned$Year <- as.factor(df_DEFRA_household_201618_cleaned$Year)

# Row bind data frames (2009 to 2018)
df_DEFRA_household_200918_cleaned <- rbind(df_DEFRA_household_200915_cleaned, df_DEFRA_household_201618_cleaned)
...
df_DEFRA_household_200915_long$DataSupplier <- "DEFRA"
df_DEFRA_household_200915_long$DataSet <- "FamilyFoods_household"
df_DEFRA_household_200915_long$Commodity <- "Purchases"
df_DEFRA_household_200915_long$Flag <- "ConV4, ConV5"
df_DEFRA_household_200915_long$TemporalResolution <- "CalendarYear"
df_DEFRA_household_200915_long$Units <- "Grams/Capita/Week"
df_DEFRA_household_200915_long$FlagDescription <- "Units changed to g/country/year"
...
# Select desired columns for cleaned data set
df_DEFRA_household_200915_cleaned <- df_DEFRA_household_200915_long[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
# Change file path to "ProcessedData"
filepath <- paste(data_dir,"ProcessedData", sep="")
write.csv(df_DEFRA_household_200918_cleaned, file =
...
df_DEFRA_eatenOut_200915 <- subset(df_DEFRA_eatenOut_200915_data, 
                           select = c("EO Minor code", "Units", "2009", "2010", "2011",
                                      "2012", "2013","2014", "2015"))
...
```{r Transform data set (2009 to 2015)}
# Change structure of the data set 
df_DEFRA_eatenOut_200915_long <- melt(df_DEFRA_eatenOut_200915, id.vars = c("EO Minor code", "Units"), variable.name = "Year", value.name = "Value")
```


```{r Join data frame with translation document}
# Duplicate species name column (allows us to check species have been renames correctly)
df_DEFRA_eatenOut_200915_long$SpeciesNEW <- df_DEFRA_eatenOut_200915_long$`EO Minor code`

colnames(df_DEFRA_eatenOut_200915_long)

# Join data frame with MCS data frame by species name
df_DEFRA_eatenOut_200915_long <- df_DEFRA_eatenOut_200915_long %>% inner_join(., MCS_translations, by = c('SpeciesNEW' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
```{r Add additional variables to data set}
# Add additional variables
df_DEFRA_eatenOut_200915_long$DataSupplier <- "DEFRA"
df_DEFRA_eatenOut_200915_long$DataSet <- "FamilyFoods_eatenOut"
df_DEFRA_eatenOut_200915_long$Commodity <- "Purchase"
df_DEFRA_eatenOut_200915_long$Flag <- "ConV4, ConV5"
df_DEFRA_eatenOut_200915_long$TemporalResolution <- "CalendarYear"
df_DEFRA_eatenOut_200915_long$Units <- "Grams/Capita/Week"
df_DEFRA_eatenOut_200915_long$FlagDescription <- "Units changed to g/country/year"
...
# Select desired columns for cleaned data set
df_DEFRA_eatenOut_200915_cleaned <- df_DEFRA_eatenOut_200915_long[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]

```


Now combine eating out data sets from years 2009 to 2018 

```{r Combine DEFRA eating out data i) 2009 to 2015; ii) 2016 to 2018}
# Convert "Year" to a factor value
df_DEFRA_eatenOut_200915_cleaned$Year <- as.factor(df_DEFRA_eatenOut_200915_cleaned$Year)
df_DEFRA_eatenOut_201618_cleaned$Year <- as.factor(df_DEFRA_eatenOut_201618_cleaned$Year)

# Combine DEFRA eating out data between 2009 to 2015 and 2016 to 2018
df_DEFRA_eatenOut_200918 <- rbind(df_DEFRA_eatenOut_200915_cleaned, df_DEFRA_eatenOut_201618_cleaned)
...
# Change file path to "ProcessedData"
filepath <- paste(data_dir,"ProcessedData", sep="")
write.csv(df_DEFRA_eatenOut_200918, file =
...
```{r Load data (2004)}
# Read in .csv file 
df_PHE_composition_data <- vroom(file=paste(filepath,"PHE_SeafoodNutrientContent.csv", sep="/"), na = c("N/A", "Tr")) # Both N/A and Tr different 
...
df_PHE_composition_data$Commodity <- "NutrientComposition"
df_PHE_composition_data$Flag <- "NA"
df_PHE_composition_data$FlagDescription <- "NA"

```


The main nutrients in fish are calcium, iron, selenium, zinc, vitamin D and omega-3 fatty acids. 
...
# Select desired columns for cleaned data set
df_nutcomp_PHE_cleaned <- df_PHE_composition_data[c("SampleDescription", "RevisedMCS", "EUMOFA_ST", "FAO_ST", "ISSCAAP_ST", "Golden_2021_ST","Bianchi_2022_ST", "WWF_2022_ST", "SACN", "DataSupplier", "DataSet", "Commodity", "Flag", "Calcium-mg100g", "Iron-mg100g", "Selenium-µg100g", "Zinc-mg100g", "TotalPUFA-g100g", "VitaminD-µg100g", 
                                                    "FlagDescription", "Flag")]


#"DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription"

# Here the main nutrients have been selected but his list can be easily changed.
...
# Note to AL: this may be more Cathrine's area of work.
...
write.csv(df_nutcomp_PHE_cleaned, file =
            paste(filepath,"NutrientComposition_PHE_PreliminaryCleaned.csv", sep=""), row.names = FALSE)
