...
# Read in .csv file
df_UK_population_200918 <- vroom(file=paste(filepath,"UKPopulation_ONS_PreliminaryCleaned-edited.csv", sep="/"), na = c(".."))


# UK population data. Mid year estimates 2001910 to 201819
# Read in .csv file
df_UK_MidYearpopulation <- vroom(file=paste(filepath,"ONS_UK_MidYearPopulation-20091018.csv", sep="/"), na = c(".."))
...
df_UK_AllUKLandings_200918_clean <- df_UK_AllUKLandings_200918[c("DataSupplier", "DataSet", "Commodity", "Species.x", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
df_UK_aquaculture_200918 <- df_UKFarmed_200918[c("DataSupplier", "DataSet", "Commodity", "Species.x", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
```{r Sum seafood consumption over 4-day diet recall, by species, by year}
# Sum seafood consumption per species per year
df_NDNS_seafood_aggregate_person_Yr1_11 <- aggregate(totfishperprod ~ Species + SurveyYear + SpeciesType + SACN, data =
                                                df_NDNS_seafood_consumption_Yr1_11, sum)

```


```{r Calculate the annual seafood intake, by species}
# Divide summed seafood consumption by 4 (4-day diet recall)
...
respondersPerYear <- df_NDNS_seafood_aggregate_person_Yr1_11 %>%
    group_by(SurveyYear) %>%
    summarise(Count = n())   

# Merge NDNS responses with consumption data 
df_NDNS_seafoodReported_Yr1_11 <- merge(df_NDNS_seafood_aggregate_person_Yr1_11, respondersPerYear, by = "SurveyYear")

# Divide seafood consumption by NDNS respondents 
df_NDNS_seafoodReported_Yr1_11$avgIntakeNDNS <- df_NDNS_seafood_aggregate_person_Yr1_11$gYear / df_NDNS_seafoodReported_Yr1_11$Count

```


Before merging with population data need to change survey values. 

```{r Allocate numerical values to survey years}
# Create new data frame with survey years and corresponding numerical values
SurveyYear <- c("NDNS Year 1", "NDNS Year 2", "NDNS Year 3","NDNS Year 4","NDNS Year 5", "NDNS Year 6", "NDNS Year 7",
                "NDNS Year 8","NDNS Year 9", "NDNS Year 10", "NDNS Year 11")
NDNSYear <- c(200809, 200910, 201011, 201112, 201213, 201314, 201415, 201516, 201617, 201718, 201819)  

# Create new data frame
df_NDNSYearData <- as.data.frame(cbind(SurveyYear, NDNSYear))
df_NDNSYearData$NDNSYear <- as.numeric(as.character(df_NDNSYearData$NDNSYear))

# Create new data frame allocate numerical values by merging data frames
df_NDNS_seafood_consumption_20081919 <- merge(df_NDNS_seafoodReported_Yr1_11, df_NDNSYearData, by = "SurveyYear")

```


```{r Calcaulte seafood intake for whole of UK}
# Join seafood df_NDNS_seafood_consumption_20081919 with population data by year
df_NDNS_seafoodReported_Yr1_11 <- df_NDNS_seafood_consumption_20081919 %>% inner_join(., df_UK_MidYearpopulation, by = c('NDNSYear' = 'MidYear'))

colnames(df_NDNS_seafoodReported_Yr1_11)

# Multiply by the number of people in the country
df_NDNS_seafoodReported_Yr1_11$Value <- df_NDNS_seafoodReported_Yr1_11$avgIntakeNDNS * df_NDNS_seafoodReported_Yr1_11$Estimates
...
```{r Add additional variables to data set}
# Create "cleaned" data frame, defining units and data source 
df_NDNS_seafoodReported_Yr1_11$DataSupplier <- "NDNS"
df_NDNS_seafoodReported_Yr1_11$DataSet <- "FoodDiaries"
df_NDNS_seafoodReported_Yr1_11$Commodity <- "Consumption"
df_NDNS_seafoodReported_Yr1_11$Flag <- "ConV2"
df_NDNS_seafoodReported_Yr1_11$TemporalResolution <- "FinancialYear"
df_NDNS_seafoodReported_Yr1_11$Units <- "Grams/Country/Year"
df_NDNS_seafoodReported_Yr1_11$FlagDescription <- "Units changed to Grams/Country/Year"
...
# Select desired columns for cleaned data set
df_NDNS_seafood_consumption_Yr1_11_cleaned <- df_NDNS_seafoodReported_Yr1_11[c("DataSupplier", "DataSet", "Commodity", "Species", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
# Change structure of the data set 
df_DEFRA_household_200915_long <- melt(df_DEFRA_household_200915, id.vars = c("MinorFoodCode", "Units"), variable.name ="Year", value.name = "Value")
```
...
df_DEFRA_household_201618_to_join <- rename(df_DEFRA_household_201618_short, 
                                            FoodProduct = codeDESCRIPTION, Value = estimate)
...
# Ensure "Year" column is a numerical value. Convert to character first, then numeric
...
```{r Join data frame with translation document}
# Duplicate species name column (allows us to check species have been renames correctly)
df_DEFRA_household_200918$SpeciesNEW <- df_DEFRA_household_200918$FoodProduct

# Join data frame with MCS data frame by species name
df_DEFRA_household_200918 <- df_DEFRA_household_200918 %>% inner_join(., MCS_translations, by = c('SpeciesNEW' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
df_DEFRA_household_200918_agg <- aggregate(Value ~ Species + Year + SpeciesType + SACN, data =
                                                df_DEFRA_household_200918, sum)

#unique(df_DEFRA_household_200918_agg$Species)


#df_DEFRA_household_200918_agg <- aggregate(Value ~ Species + Year, data =
 #                                               df_DEFRA_household_200918, sum)

# Merge with translation document again to obtain species type 
#df_DEFRA_household_200918_agg$SpeciesNEW <- df_DEFRA_household_200918_agg$Species 
#df_DEFRA_household_200918_agg <- df_DEFRA_household_200918_agg %>% inner_join(., MCS_translations, by = c('SpeciesNEW' = 'MCS (DO NOT TOUCH)'))
...
# Multiply value data by 52 (convert grams from week to year)
df_DEFRA_household_200918_aggYear$PerYear <- df_DEFRA_household_200918_aggYear$Value * 52

# Create new column multiplying by population size
df_DEFRA_household_200918_aggYear$totUK <- df_DEFRA_household_200918_aggYear$PerYear * df_DEFRA_household_200918_aggYear$PopSize
...
# Rename value column- totUK will be the new "Value" column 
df_DEFRA_household_200918_aggYear$totUK
```
...
# Select desired columns for cleaned data set
df_DEFRA_household_200918_cleaned <- df_DEFRA_household_200918_aggYear[c("DataSupplier", "DataSet", "Commodity", "Species.x", "SpeciesType", "SACN", "totUK", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
