...
```{r Delete later}
df_plot_weeklyInkateYear <- aggregate(avgWeekIntakeNDNS ~ SurveyYear, data = df_NDNS_seafoodReported_Yr1_11, sum)

ggplot(df_plot_weeklyInkateYear, aes(x = SurveyYear, y = avgWeekIntakeNDNS)) +
  geom_point()
...
tot_seafoodYear <- aggregate(Value ~ Year, data = df_NDNS_seafoodintakeUK_200918_clean, sum)

ggplot(tot_seafoodYear, aes(x=Year, y= Value)) +
  geom_point()
