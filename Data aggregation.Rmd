...
MCS_translations <- vroom(file=paste(filepath,"AllMainCommercialSpecies_translations-edit.csv", sep="/"))
...
# Read in .csv file
df_UK_population_200918 <- vroom(file=paste(filepath,"UKPopulation_ONS_PreliminaryCleaned-edit.csv", sep="/"), na = c(".."))
...
df_UK_AllUKLandings_200918_clean <- df_UK_AllUKLandings_200918[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
df_UK_aquaculture_200918 <- df_UKFarmed_200918[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
# Select desired columns for cleaned data set
df_cleaned_UK_HMRC_200919 <- df_trade_HMRC_data[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
filepath <- paste(data_dir,"ProcessedData", sep = "")
write.csv(df_cleaned_UK_HMRC_200919_cleaned, file = paste(filepath,"Trade_HMRC_clean-edit.csv", sep = ""), row.names = FALSE)
...
df_DEFRA_household_200918_agg <- aggregate(Value ~ SeafoodSpecies + Year + SpeciesType + SACN, data =
...
df_DEFRA_household_200918_aggYear_cleaned <- rename(df_DEFRA_household_200918_cleaned, 
                                                    Species = SeafoodSpecies, Value = totUK)
...
```{r Transform data set (2009 to 2015)}
# Change structure of the data set 
df_DEFRA_eatenOut_200915_long <- melt(df_DEFRA_eatenOut_200915, id.vars = c("EO_MinorCode", "Units"), variable.name = "Year", value.name = "Value")
...
# Rename variables to allow data frames to merge
# Rename codeDescription and estimate column
df_DEFRA_eatenOut_201618_to_join <- rename(df_DEFRA_household_201618_short, 
                                            FoodProduct = codeDESCRIPTION, Value = estimate)
...
```{r Join data frame with translation document}
# Duplicate species name column (allows us to check species have been renames correctly)
df_DEFRA_eatenOut_200915_long$SpeciesRevised <- df_DEFRA_eatenOut_200915_long$EO_MinorCode

# Join data frame with MCS data frame by species name
df_DEFRA_eatenOut_200915_long <- df_DEFRA_eatenOut_200915_long %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.
...
# Duplicate species name column (duplicated column entries to be duplicated)
df_PHE_composition_data$SpeciesRevised <- df_PHE_composition_data$SampleDescription

# Merge data frame with MCS data frame by species name
df_PHE_composition_data <- df_PHE_composition_data %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
