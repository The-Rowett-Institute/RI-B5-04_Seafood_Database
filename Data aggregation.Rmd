...
df_UK_AllUKLandings_201721_data <- vroom(file=paste(filepath,"MMO_UKV_landingUKAbroad-201721.csv", sep="/"), na = c(".."))
...
# Read in .csv file
df_UK_AllUKLandings_201216_data <- vroom(file=paste(filepath,"MMO_UKV_landingUKAbroad-201216.csv", sep="/"), na = c(".."))
...
# Read in .csv file
df_UK_AllUKLandings_200811_data <- vroom(file=paste(filepath,"MMO_UKV_landingUKAbroad-200913.csv", sep="/"), na = c(".."))
...
str(df_UK_AllUKLandings_200918_clean)
...
write.csv(df_UK_AllUKLandings_200918_cleaned, file = paste(filepath,"UKVesslesLandingsintoUKAbroad_MMO_PreliminaryCleaned.csv", sep = ""), row.names = FALSE)
...
```{r Transform data set (2017 to 2018)}
# Change structure of the data set 
df_UKV_LandingUKAbroad_201718_long <- melt(df_UKV_LandingUKAbroad_201718_data, id.vars = c("Species"), variable.name = "Year", value.name = "Value")
```
...
```{r Transform data set (2012 to 2016)}
# Change structure of the data set 
df_UKV_LandingUKAbroad_201216_long <- melt(df_UKV_LandingUKAbroad_201216_data, id.vars = c("Species"), variable.name = "Year", value.name = "Value")
```
...
```{r Transform data set (2009 to 2011)}
# Change structure of the data set 
df_UKV_LandingUKAbroad_200911_long <- melt(df_UKV_LandingUKAbroad_200911_data, id.vars = c("Species"), variable.name = "Year", value.name = "Value")
...
# Join data frame with translation document- ensures consistent spelling across data sets
# Duplicate species name column (allows us to check species have been renames correctly)
df_UKV_LandingUKAbroad_200918_long$SpeciesRevised <- df_UK_AllUKLandings_200918$Species
...
df_trade_HMRC_data <- vroom(file=paste(filepath,"TradeData_HMRC_PreliminaryCleaned.csv", sep = "/"))
...
# Select desired columns for cleaned data set
df_cleaned_UK_HMRC_200919 <- df_trade_HMRC_data[c("DataSupplier.y", "DataSet", "Commodity", "SeafoodSpecies", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
df_NDNS_seafood_aggregate_person_Yr1_11 <- aggregate(totfishperprod ~ SeafoodSpecies + SurveyYear + SpeciesType + 
                                                       SACN, data = df_NDNS_seafood_consumption_Yr1_11, sum)
...
df_NDNS_seafoodintakeUK_200918$Value <- df_NDNS_seafoodintakeUK_200918$avgIntakeNDNS * df_NDNS_seafoodintakeUK_200918$UKPOP
...
df_NDNS_seafoodintakeUK_200918$TemporalResolution <- "FinancialYear"
...
# Select desired columns for cleaned data set
df_NDNS_seafoodintakeUK_200918_cleaned <- df_NDNS_seafoodintakeUK_200918[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]
...
```{r Seafood consumption (g/capita/week), by species}
# Sum seafood consumption per person per species
df_DEFRA_eatenOut_200918_agg <- aggregate(Value ~ SeafoodSpecies + Year + SpeciesType + SACN, data =
...
# Multiply value data by 52 (convert grams from week to year)
df_DEFRA_eatenOut_200918_aggYear$PerYear <- df_DEFRA_eatenOut_200918_aggYear$Value * 52
