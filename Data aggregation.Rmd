...
df_UKV_LandingUKonly_200918_cleaned %>% select(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% group_by(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% 
...
# Read in .csv file 
df_EUROSTAT_UK_aquaculture_data <- vroom(file=paste(filepath,"EUROSTAT_UK_aquaculture-200918_UPDATED.csv", sep="/"), na = c(":"))
...
```{r Check for duplicates}
str(df_UK_aquaculture_200918_cleaned)

df_UK_aquaculture_200918_cleaned %>% select(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% group_by(DataSupplier, DataSet, Commodity, Species, Year, Volume, Units) %>% 
  filter(n()>1) %>% mutate(n = n())
