...
### A) i) Capture fisheries (MMO) UK and foregin vessels landing in to the UK
...
MMOSpeciesALLUKLandings <- as.data.frame(unique(df_UK_AllUKLandings_200918$Species))

# Change file path to "ProcessedData". Can also use setwd()
filepath <- paste(data_dir,"ProcessedData/", sep = "")

# Stating row.names = FALSE removes the initial column of the new .csv file where the rows are numbered
write.csv(MMOSpeciesALLUKLandings, file = paste(filepath,"UniqueMMOSpecies.csv", sep = ""), row.names = FALSE)
...
#df_UK_AllUKLandings_200918_cleaned <- rename(df_UK_AllUKLandings_200918_clean, c("SeafoodSpecies" = "Species"))
#df_UK_AllUKLandings_200918_cleaned <- rename(df_UK_AllUKLandings_200918_clean, SeafoodSpecies = Species)

df_UK_AllUKLandings_200918_cleaned <- df_UK_AllUKLandings_200918_clean %>% rename(Species = SeafoodSpecies)
...
### A) ii) Capture fisheries (MMO) UK vessels landing in to the UK and abroad
...
# Select desired variables and years (2017 to 2019)
df_UKV_LandingUKAbroad_201718_data <- df_UKV_LandingUKAbroad_201721_data[c("Species", "2017", "2018")]
```
...
```{r DELETE LATER}
# Create dataframe of unique species 
MMOSpeciesUKVesselsLandings <- as.data.frame(unique(df_UKV_LandingUKAbroad_200918_long$Species))

# Change file path to "ProcessedData". Can also use setwd()
filepath <- paste(data_dir,"ProcessedData/", sep = "")

# Stating row.names = FALSE removes the initial column of the new .csv file where the rows are numbered
write.csv(MMOSpeciesUKVesselsLandings, file = paste(filepath,"UniqueMMOSpeciesUKVessels.csv", sep = ""), row.names = FALSE)
...
df_UKV_LandingUKAbroad_200918_long$DataSet <- "Sea Fisheries Statistics"
...
df_UKV_LandingUKAbroad_200918_cleaned <- df_UKV_LandingUKAbroad_200918_clean %>% rename(Species = SeafoodSpecies)
...
# Stating row.names = FALSE removes the initial column of the new .csv file where the rows are numbered
write.csv(df_UKV_LandingUKAbroad_200918_cleaned, file = paste(filepath,"UKVessels_LandingUKAbroad_MMO_PreliminaryCleaned.csv", sep = ""), row.names = FALSE)
...
# Select desired variables and years (2017 to 2019)
df_UKV_LandingUKonly_201718_data <- df_UKV_LandingUKonly_201721_data[c("Species", "2017", "2018")]
...
```{r Transform data set (2017 to 2018)}
# Change structure of the data set 
df_UKV_LandingUKonly_201718_long <- melt(df_UKV_LandingUKonly_201718_data, id.vars = c("Species"), variable.name = "Year", value.name = "Volume")
```
...
```{r Combine data sets: i) 2008 to 2011; ii) 2012 to 2016; and iii) 2017 to 2019)}
# Combine capture production data i) 2009 to 2011; ii) 2012 to 2016; and iii) 2017 to 2020)
df_UKV_LandingUKonly_200918_long <- rbind(df_UKV_LandingUKonly_201718_long,
...
# Join data frame with translation document- ensures consistent spelling across data sets
# Duplicate species name column (allows us to check species have been renames correctly)
df_UKV_LandingUKonly_200918_long$SpeciesRevised <- df_UKV_LandingUKonly_200918_long$Species

# Join data frame with MCS data frame by species name
df_UKV_LandingUKonly_200918_long <- df_UKV_LandingUKonly_200918_long %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
# MSC (DO NOT TOUCH) column is the unique spelling of the species in all data sets.

# Check column names of the updated data frame
colnames(df_UKV_LandingUKonly_200918_long)
...
```{r Apply conversion factors to calculate edible portion}
# Identify unique conversion factors in the data set
unique(df_UKV_LandingUKonly_200918_long$CF_value) # no processed data here so no 1 Volumes

# Create new column and apply conversion factors Volumes
df_UKV_LandingUKonly_200918_long$apply_CF <- df_UKV_LandingUKonly_200918_long$Volume * df_UKV_LandingUKonly_200918_long$CF_value
...
```{r Convert from 1000 tonnes to grams}
# Convert raw Volume from 000's tonnes to grams 
df_UKV_LandingUKonly_200918_long$Volume <- (df_UKV_LandingUKonly_200918_long$apply_CF * 1000) * 1000000
```


```{r Add additional variables to data set}
# Create "cleaned" data frame, defining units and data source 
df_UKV_LandingUKonly_200918_long$Units <- "Grams"
df_UKV_LandingUKonly_200918_long$DataSupplier <- "MMO"
df_UKV_LandingUKonly_200918_long$DataSet <- "Sea Fisheries Statistics"
df_UKV_LandingUKonly_200918_long$Commodity <- "Production"
df_UKV_LandingUKonly_200918_long$Flag <- "ConV1"
df_UKV_LandingUKonly_200918_long$TemporalResolution <- "CalendarYear"
df_UKV_LandingUKonly_200918_long$FlagDescription <- "CF applied to all or part of the data"
...
# Select desired columns for cleaned data set-- 16  variables
df_UKV_LandingUKonly_200918_clean <- df_UKV_LandingUKonly_200918_long[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "Volume", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]

# Check column names
colnames(df_UKV_LandingUKonly_200918_clean)

# Rename "SeafoodSpecies" column 
#df_UKV_LandingUKAbroad_200918_cleaned <- rename(df_UKV_LandingUKAbroad_200918_clean, c("SeafoodSpecies" = "Species"))

df_UKV_LandingUKonly_200918_cleaned <- df_UKV_LandingUKonly_200918_clean %>% rename(Species = SeafoodSpecies)
...
# Stating row.names = FALSE removes the initial column of the new .csv file where the rows are numbered
write.csv(df_UKV_LandingUKonly_200918_cleaned, file = paste(filepath,"OnlyUKVessels_LandingUK_MMO_PreliminaryCleaned.csv", sep = ""), row.names = FALSE)
...
df_UKFarmed_200918$DataSet <- "Aquaculture excluding hatcheries and nurseries "
...
df_UK_aquaculture_200918_cleaned <- df_UK_aquaculture_200918 %>% rename(Species = SeafoodSpecies)
...
# Remove "SpeciesType" and "SACN" variables, to prevent duplication when merging with updated species classification document 
df_trade_HMRC_data_edit <- subset(df_trade_HMRC_data, select = - c(SpeciesType, SACN, Flag, TemporalResolution, FlagDescription, DataSupplier))
colnames(df_trade_HMRC_data_edit)
...
```{r Join data frame with translation document}
# Duplicate species name column (duplicated column entries to be duplicated)
df_trade_HMRC_data_edit$SpeciesRevised <- df_trade_HMRC_data_edit$Species

# Merge data frame with MCS data frame by species name
df_trade_HMRC_data_edit <- df_trade_HMRC_data_edit %>% inner_join(., MCS_translations, by = c('SpeciesRevised' = 'MCS (DO NOT TOUCH)'))
...
df_trade_HMRC_data_edit$Flag <- "ConV1"
df_trade_HMRC_data_edit$TemporalResolution <- "CalendarYear"
df_trade_HMRC_data_edit$FlagDescription <- "CF applied to all or part of the data"

# Select desired columns for cleaned data set
df_cleaned_UK_HMRC_200919 <- df_trade_HMRC_data_edit[c("DataSupplier", "DataSet", "Commodity", "SeafoodSpecies", "SpeciesType", "SACN", "Value", "Units", "Year", "TemporalResolution", "Flag", "FlagDescription")]

# Rename "SeafoodSpecies" column to "Species"
df_cleaned_UK_HMRC_200919_cleaned <- df_cleaned_UK_HMRC_200919 %>% rename(Species = SeafoodSpecies, Volume = Value)

#df_cleaned_UK_HMRC_200919_clean <- rename(df_cleaned_UK_HMRC_200919, c("SeafoodSpecies" = "Species"))
#df_cleaned_UK_HMRC_200919_cleaned <- rename(df_cleaned_UK_HMRC_200919_clean,c("Value" = "Volume"))
...
#unique(df_cleaned_UK_HMRC_200919_cleaned_again$Species) # fishmeal present here
...
unique(df_DEFRA_household_201618_short$codeDESCRIPTION)

df_DEFRA_household_201618_short$Units <- "g"
...
df_DEFRA_household_200918_aggYear$DataSet <- "Family Foods Household"
...
df_DEFRA_household_200918_aggYear$Units <- "Grams"
df_DEFRA_household_200918_aggYear$FlagDescription <- "Unit converted from g/capita/week to g/country/year"
...
```{r Delete later}
# Ascertain annual seafood purchases
#annual_purchase_201620 <- aggregate(estimate ~ Year, data = df_DEFRA_purchase_201620_data, sum)
#annual_purchase_HH_200915 <- aggregate(Volume ~ Year, data = df_DEFRA_household_200915_long, sum)
#annual_purchase_EO_200915 <- aggregate(Volume ~ Year, data = df_DEFRA_eatenOut_200915_long, sum)

#annual_purchases_HH_201618 <- aggregate(estimate ~ Year, data = df_DEFRA_household_201618, sum)
#annual_purchases_EO_201618 <- aggregate(estimate ~ Year, data = df_DEFRA_eatenOut_201618, sum)

# Households
#annual_purchase_HH_200915J <- aggregate(Volume ~ Year, data = df_DEFRA_household_201915_to_join, sum)
#annual_purchase_HH_201618J <- aggregate(Volume ~ Year, data = df_DEFRA_household_201618_to_join, sum)
#annual_purchase_HH_200918 <- aggregate(Volume ~ Year, data = df_DEFRA_household_200918, sum)

#annual_household_200918 <- aggregate(Volume ~ Year, data = df_DEFRA_household_200918_agg, sum)
...
# Allocate units
df_DEFRA_eatenOut_201618_short$Units <- "g/capita/week"
...
df_DEFRA_eatenOut_200918_aggYear$DataSet <- "Family Foods Eaten Out"
...
df_DEFRA_eatenOut_200918_aggYear$Units <- "Grams"
df_DEFRA_eatenOut_200918_aggYear$FlagDescription <- "Unit converted from g/capita/week to g/country/year"
