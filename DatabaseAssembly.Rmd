...
# Merge seafood database with nutrient composition data
df_seafoodCompDB <- merge(df_Seafood_DB, df_Nutrie, "Species", all= TRUE)

df_seafoodCompDB <- merge(df_Nutrie, df_Seafood_DB, "Species")
...
write.csv(df_seafoodCompDB, paste(filepath,"Seafood-database_preliminary.csv",sep="/"), row.names = FALSE)
```
...
# Save datframe to "Outputs" data file
filepath <- paste(data_dir,"Outputs", sep="")
write.csv(df_UK_seafoodDB_capita, paste(filepath,"Seafood-database-per-capita_preliminary.csv",sep="/"), row.names = FALSE)
```
